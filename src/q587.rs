...
pub fn outer_trees(trees: Vec<Vec<i32>>) -> Vec<Vec<i32>> {
    ...
    loop {
        ...
        for tree in trees.iter() {
            let o =
                orientation([on_hull, next_point, tree]);
            if o == Orientation::CounterClockwise
                || next_point[0] == on_hull[0]
                    && next_point[1] == on_hull[1]
            {
                next_point = tree;
            } else if o == Orientation::Colinear
                && dist([on_hull, tree])
                    > dist([on_hull, next_point])
            {
                on_hull = next_point;
                next_point = tree;
            }
